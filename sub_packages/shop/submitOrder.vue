<template>
  <view class="submit-order-page">
    <visitNotice ref="notice" :fontMode="fontMode" :noticeType="noticeType" @confirmed="handleConfirm" />
    ...
    <view class="footer-bar">
      ...
      <button class="pay-button" @click="submitOrder" :disabled="!canPay" :class="{ 'disabled-btn': !canPay }">
        在线审方
      </button>
    </view>
      ...
      <view class="address-popup">
        ...
        <scroll-view scroll-y class="address-list">
          <view v-for="(item, index) in addressList" :key="index" class="address-item"
            :class="{ active: item.id === selectedAddress.id }" @click="selectAddress(item)">
    ...
    <uni-popup ref="notePopup" type="bottom" background-color="#fff">
      ...
      <view class="popup-content">
        <textarea v-model="orderData.note" placeholder="请输入您的备注内容" maxLength="100" class="note-textarea"></textarea>
      </view>
