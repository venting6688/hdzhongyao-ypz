<template>
  <view class="shop-detail-page">
    ...
    <scroll-view class="shop-detail-content" scroll-y :style="{
        'margin-top': barHeight + 'px',
        'margin-bottom': footData.isShow ? footData.height + 'px' : '0px'
      }">
      <view class="drug-image">
        <swiper class="swiper" circular :indicator-dots="false" :autoplay="false" :interval="interval"
          :duration="duration">
          <swiper-item>
  ...
  export default {
    ...
    data() {
      return {
        ...
        serviceText: '药品配送业务由第三方快递公司承接，由不可预知的缘故导致的药品配送延误，由患者本人与快递公司协商解决。药品离柜进入配送环节之后，无特殊原因，不予退换。'
      }
    },
    filters: {
      formatPrice(value) {
    ...
    methods: {
      ...
      async fetchDrugDetail(drugId) {
        const res = await shop.getDrugDetailApi({
          drugId,
          userId: 23
        })
        if (res && res.data) {
