  ...
  <view class="information">
    <form>
      ...
      <view class="cu-form-group" style="margin-top: 30rpx">
        ...
        <input
          v-model="informationObj.phone"
          placeholder="请输入院内预留手机号"
          type="number"
          maxlength="11"
          name="input"
        />
      ...
      <view class="cu-form-group">
        ...
        <input
          v-model="informationObj.verificationCode"
    ...
    <view
      ...
      :class="{
        unclickable:
          !informationObj.patientName ||
          !informationObj.idNo ||
          !informationObj.phone ||
          !informationObj.verificationCode
      }"
    >
      添加就诊人
    </view>
...
import { getAge } from '@/utils/system'
export default {
  mixins: [mixin],
  data() {
    return {
  ...
  methods: {
    ...
    verificationCodeBtn() {
      ...
      this.count(60)
      filingApi
        .sendVerificationCode(this.informationObj.phone)
        .then(res => {})
        .catch(err => {
          console.log('2：', err)
        })
    ...
    Filing() {
      if (
        !this.informationObj.patientName ||
        !this.informationObj.idNo ||
        !this.informationObj.phone ||
        !this.informationObj.verificationCode
      ) {
        uni.showToast({
          title: '请完善您的信息',
          icon: 'none',
    ...
    async refreshUserInfo() {
      try {
        const res = await familyApi
          .getDefaultPatientApi({ ownerUserId: this.loginValue?.userId })
          .then(res => {
