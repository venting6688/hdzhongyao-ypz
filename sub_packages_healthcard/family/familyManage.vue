      ...
      <ul v-if="healthCardList.length">
        <li v-for="(item, index) in healthCardList" :key="item.archives.archiveId">
          <view class="title">
            <view class="name">
              <text class="big">
                {{ item.archives.patientName ? pixelate(item.archives.patientName) : '' }}
              </text>
            ...
            <view class="dele" @click="deletePatients(item.archives.familyId, item.archives.defaultType)">
    ...
    <uni-popup ref="popup" type="center">
      <view class="auth-popup" v-if="showAuth">
        ...
        <view class="auth-content">
          <view>
            即将使用电子健康卡服务，
            <span style="color: #1b98ff">开启授权，就医一卡通行</span>
          </view>
        </view>
        <health-card-login :wechatcode="true" @authFail="authFail" @authSuccess="authSuccess" @authCancel="authCancel">
    ...
    methods: {
      ...
      async getHealthCard() {
        ...
        if (res.data.code == 200) {
          ...
          let hisCardType =
            healthVal == '居民户口簿' ? 1 : filterHisCard.length > 0 ? filterHisCard[0].index : 98
          ...
          this.healthCardList =
            rows?.map((item, index) => ({
              ...
              healthCard: {
                name: item.real_name,
                idCard: item.id_card ?
                  `${item.id_card.slice(0, 4)}${'*'.repeat(
                    item.id_card.length - 10
                  )}${item.id_card.slice(-2)}` :
                  '',
                idNo: item.id_card,
      ...
      async defaultPatients(val) {
        const {
          code,
          msg
        } = await family.setDefaultMemberApi({
